...
def runTestFour(output_dir, test_file, output_file_name):
    ...
    for ((i, j), k) in new_prob:
        counter += 1
        nvp_out = output_dir + "output_test4_" + str(counter) + ".txt"
        runNvp(4, test_dir + test_file, nvp_out, output_file, str(k), str(i), str(j), str(counter))
    petsc_out = output_dir + "petsc_out.txt"
    runPETSc(test_dir + test_file, petsc_out, output_dir+output_file_name)
